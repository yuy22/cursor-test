...
INPUT   = Path(__file__).parent / "你的文件.docx"   # ← 改成你的文件路径
OUT_DIR = Path(__file__).parent / "output" / "书名_cleaned"  # ← 改成输出目录
BOOK_TITLE   = "书名"        # ← 出现在 MD 文件头的书名
BOOK_AUTHOR  = "作者名"      # ← 出现在 MD 文件头的作者
...
SIZE_TO_HEADING = {
    # 示例（按实际字号分析结果填写）：
    # 23.5: 2,   # "教育是一门艺术"（大章标题）
    # 16.0: 3,   # "1998—2002 忽如一夜春风来"（时期小节）
    # 14.5: 4,   # "◎ 学生对我影响很大"（访谈小节）
    # 13.5: 4,   # "访谈手记"（固定小标签）
}
...
# 本书特有的页眉/页脚模式（正则），命中即删整行
# 通常是"书名 页码"或"作者 书名"格式
BOOK_HEADER_PATTERNS = [
    # 示例：re.compile(r'^\d+\s+俞正强\s*[：:]\s*低头找'),
    # 示例：re.compile(r'^低头找幸福\s*$'),
]
